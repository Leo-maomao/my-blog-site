...
-- 7. 创建Banner表
CREATE TABLE IF NOT EXISTS blog_banners (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  title TEXT NOT NULL,
  description TEXT,
  badge TEXT,
...
-- 8. 创建Banner索引
CREATE INDEX IF NOT EXISTS idx_blog_banners_active ON blog_banners(active);
CREATE INDEX IF NOT EXISTS idx_blog_banners_position ON blog_banners(position);

-- 9. 配置Banner表的RLS
ALTER TABLE blog_banners ENABLE ROW LEVEL SECURITY;

-- 允许所有人读取激活的Banner
DROP POLICY IF EXISTS "Public read active banners" ON blog_banners;
CREATE POLICY "Public read active banners"
ON blog_banners FOR SELECT
USING (active = true);

-- 只有认证用户可以管理Banner
DROP POLICY IF EXISTS "Authenticated users manage banners" ON blog_banners;
CREATE POLICY "Authenticated users manage banners"
ON blog_banners FOR ALL
USING (auth.role() = 'authenticated');

-- 10. Banner更新时间自动触发器
DROP TRIGGER IF EXISTS update_blog_banners_updated_at ON blog_banners;
CREATE TRIGGER update_blog_banners_updated_at
    BEFORE UPDATE ON blog_banners
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();
...
-- 4. 在 Storage 中创建 blog-banners bucket（公开访问）
-- =====================================================
